...
private:
    void readAudioProperties(const TagLib::File *file, const TrackTags &result) const;
    void readGenericMetadata(const TagLib::PropertyMap &properties, TrackTags &result) const;

    bool readGenericField(const TagLib::PropertyMap &properties, const std::string &tagName,
                          MetadataFields::ColumnsRoles role, TrackTags &result) const;

    void readID3v2Tags(const TagLib::ID3v2::Tag *id3tags, TrackTags &result) const;
